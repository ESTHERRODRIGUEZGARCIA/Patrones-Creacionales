...
def main():
    ...
    while True:
        ...
        if choice == "1":
            pizza_number = input("Elige el número de la pizza que deseas (1-10): ")
            if pizza_number.isdigit() and 1 <= int(pizza_number) <= 10:
                pizza_info = menu[int(pizza_number) - 1]
                print(f"Has elegido: {pizza_info['name']}")
                pizza_builder = PizzaBuilder()
                pizza_builder.set_name(pizza_info['name'])
                pizza_builder.set_category(pizza_info['category'])
                pizza_builder.set_ingredients(pizza_info['ingredients'])
